...
3. **分析**：点击右上角「分析」按钮后：
   - 从**网页内容**中获取「以字母/数字开头的完整标题」（不使用浏览器标签页标题）。
     候选优先级：站点专属（javdb.com：**优先 `<h2 class="title is-4">` 结构**（层级最少），
     内含 `savdiv.sav-id`（番号）+ `strong.current-title`（标题），取 h2 完整文本如
     "AKDLD-363 【...】若葉結希"，只有不存在标题标签候选时才回退父元素，避免列表页
     `div.video-title` 等过长文字被当标题；
     javbus.com：`<h3>` 内的 `savdiv.sav-id` + 标题文字，取 h3 完整文本如
     "START-607 おねだり...恋渕ももな"；两者都只保留以字母/数字开头的内容）→
     title 类元素 / 标题元素 / 链接文字 / 文本块，显示在弹框最上面；
   - **115 查重**：用标题最前面的字母-数字（番号，如 `MIAB-481`）GET 调用
     ...
     则显示**红色「⚠️ 番号xxx已存在<所在目录>中」**（115 data 字段：文件用 `dp` 父目录名、
     目录用自身 `n`），否则显示**蓝色「番号xxx未保存」**；右上角按钮同样显示该状态；
   - 弹框按行展示页面所有下载链接（**magnet 磁力 + ed2k 电驴**，名称 + 大小），
     **按大小从大到小排序**，每条带「转存」按钮；
   - **动态内容自动刷新**：MutationObserver + 定时重扫（弹框打开时每 1.5 秒比对链接集合），
     页面切换标签、SPA 导航、AJAX 等新加载的 magnet/ed2k 链接都会自动进入列表；
   - **多方式提取**：标准 `<a href>` 锚点 + 文本节点 + `data-clipboard-text` 等属性 +
     容器 textContent（URL 被标签拆开时也能拼出）+ 同源 iframe；
   - 标题候选会**排除下载链接（magnet/ed2k）相关文本**，避免 ed2k 磁力被当成标题；
   - 无下载链接时弹框提示 **无资源**；
3. **转存**：点击「转存」后：
...
4. **完成检测与处理**（后台异步，用户规范）：
   ...
   - **轮询任务状态**：`POST https://115.com/web/lixian/?ct=lixian&ac=task_lists`
     ...
     磁力链接相同 → 任务未完成（记录 `delete_file_id`），继续轮询，**最多 30 秒**，超时提示下载超时；
     下载中列表没有本次任务 → 改 `stat=11` 查询已完成任务，若 `url` 匹配 → 下载完成，取 `delete_file_id`；
     两个列表都没有 → 提示**下载失败**；
   - **按 delete_file_id 重命名**：在「云下载」目录中按任务的 `delete_file_id`（作为文件 id）
     定位下载产物——**目录** → 进入并重命名其中最大的文件，然后**将该文件移动到云下载目录**；
...
- **不影响页面图片加载**：DNR 头注入规则**只匹配 fetch 请求**（xmlhttprequest/other），
  绝不作用于页面 `<img>` 等图片请求；SW 启动时会自动清理遗留的 DNR 会话规则，
  避免规则残留干扰 javdb/javbus 的 CDN 图片加载（如 ERR_ADDRESS_UNREACHABLE）。
- **按钮可拖动**：右上角「分析」按钮（含"番号xxx已存在/未保存"状态）可按住拖动到任意位置，
  位置会保存并跨页面生效（`chrome.storage.local`）；拖拽结束后不会误触发分析弹框。
- **图片获取自动重试**：偶发网络失败（如 `Failed to fetch`）时，内容脚本 fetch 重试 1 次、
  后台兜底最多重试 3 次（延迟 1s/2s，javbus 最后一次去掉注入改直连）；
- **按站点注入 Referer**：仅 **javbus** 需要注入 `Referer`（用户规范：`Referer: cur_url`）；
  **javdb 不注入任何头**（其 CDN 不接受注入，按页面自然请求即可），内容脚本与后台均按站点区分。
